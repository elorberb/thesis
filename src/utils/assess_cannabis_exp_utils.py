...
def get_class_distribution(json_data):
    return {
        ...
        "clear_normalized": np.round(
            json_data["normalized_class_distribution"].get("1", 0), 4
        ),
        "cloudy_normalized": np.round(
            json_data["normalized_class_distribution"].get("2", 0), 4
        ),
        "amber_normalized": np.round(
            json_data["normalized_class_distribution"].get("3", 0), 4
        ),
    }
                ...
                for numbered_folder in os.listdir(subfolder_path):
                    ...
                    if os.path.isdir(numbered_folder_path):
                        json_file_path = os.path.join(
                            numbered_folder_path, "class_distribution.json"
                        )
                            ...
                            with open(json_file_path, "r") as f:
                                ...
                                for image_folder in os.listdir(numbered_folder_path):
                                    image_folder_path = os.path.join(
                                        numbered_folder_path, image_folder
                                    )
                                    if os.path.isdir(
                                        image_folder_path
                                    ) and image_folder.startswith("IMG_"):
                                        for img_file in os.listdir(image_folder_path):
                                            if img_file.endswith(
                                                "_class_distribution.json"
                                            ):
                                                img_json_file_path = os.path.join(
                                                    image_folder_path, img_file
                                                )
                                                with open(
                                                    img_json_file_path, "r"
                                                ) as img_f:
...
if __name__ == "__main__":
    ...
    save_data_to_csv(
        data_per_folder,
        os.path.join(output_folder, "collected_class_distribution_per_folder.csv"),
    )

    print(
        "Data collection complete. CSV files saved as 'collected_class_distribution_per_image.csv' and 'collected_class_distribution_per_folder.csv'."
    )
