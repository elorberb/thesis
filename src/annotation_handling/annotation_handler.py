...
def setup_yolo_dataset_directory(image_dir, label_dir, output_base_dir):
    ...
    # List all images
    all_images = [f for f in os.listdir(image_dir) if os.path.isfile(os.path.join(image_dir, f)) and (f.endswith('.jpg') or f.endswith('.png'))]
...
def convert_coco_to_yolo_format_from_single_dataset(annotations_folder_name, dataset_version, saving_yaml_path, train_percentage=0.8):
    """
...
def convert_coco_to_yolo_format_from_train_test_datasets(
    ...
    train_annotations_dir = f"{config.SEGMENTS_FOLDER}/{train_annotations_folder_name}/annotations"
    ...
    test_annotations_dir = f"{config.SEGMENTS_FOLDER}/{test_annotations_folder_name}/annotations"
    ...
    convert_coco(
        labels_dir=train_annotations_dir,
    ...
    convert_coco(
        labels_dir=test_annotations_dir,
        save_dir=test_output_dir,
        use_segments=False
    )
    
    # Split data into training and validation sets for training dataset
    setup_yolo_dataset_directory(train_image_dir, train_label_dir, train_organized_dataset_path)
    setup_yolo_dataset_directory(test_image_dir, test_label_dir, test_organized_dataset_path)
    ...
    yaml_file_path = os.path.join(saving_yaml_path, f"{train_annotations_folder_name}.yaml")
