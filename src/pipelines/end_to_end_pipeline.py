...
def process_image(image_path, detection_model, patch_size=512):
    ...
    logger.info(f"Time taken to process image {os.path.basename(image_path)}: {processing_time:.2f} seconds")
    return result
...
def save_class_distribution(class_distribution, normalized_class_distribution, output_path):
    ...
    logger.info(f"Class and normalized class distributions saved to JSON: {output_path}")
...
def process_images_in_folder(folder_path, detection_model, output_dir, patch_size):
    ...
    for image_file in image_files[1:]:
        ...
        normalized_class_distribution = compute_normalized_class_distribution(class_distribution)
        ...
        save_class_distribution(class_distribution, normalized_class_distribution, distribution_json_path)
...
def compute_aggregated_class_distribution(aggregated_results, output_dir):
    ...
    normalized_aggregated_class_distribution = compute_normalized_class_distribution(aggregated_class_counts)
    
    # Save combined distributions to JSON
    aggregated_class_distribution_json_path = os.path.join(output_dir, "class_distribution.json")
    save_class_distribution(aggregated_class_counts, normalized_aggregated_class_distribution, aggregated_class_distribution_json_path)

def process_all_folders(parent_folder_path, detection_model, output_base_dir, patch_size):
    ...
    for folder_path in subfolders:
        ...
        process_images_in_folder(folder_path, detection_model, output_base_dir, patch_size)
...
def main():
    ...
    parent_input_folder = "/home/etaylor/images/assessing_cannabis_experiment_images/day_3_2024_06_06/lab"
    output_base_folder = "/home/etaylor/code_projects/thesis/assessing_cannabis_exp_results/day_3_2024_06_06/lab"
    ...
    process_all_folders(parent_input_folder, detection_model, output_base_folder, patch_size)
