...
def segment_and_classify_image(image_path, segmentation_model, save_dir, classification_model=None, verbose=False, plot=False):
    if verbose:
    ...
    if np.all(segmentation_bitmap == 0):
        print("No segments found by the model for image path = {image_path}. Skipping this image.")
        return
    ...
    if plot:
        ...
        elif not classification_model:  # If no classification model provided, just plot bboxes
            if verbose:
                print("Plotting image with bounding boxes")
            plot_with_bboxes(image, bboxes)


def batch_process_images(image_dir, segmentation_model, base_save_dir, classification_model=None, verbose=False, plot=False):
    ...
    for root, dirs, files in os.walk(image_dir):
        for file in files:
            if file.endswith(('.jpg', '.png', '.jpeg')):  # Add more extensions if needed
                ...
                save_dir = os.path.join(base_save_dir, relative_path, os.path.splitext(file)[0])
                ...
                segment_and_classify_image(image_path, segmentation_model, save_dir, classification_model, verbose=verbose, plot=plot)
